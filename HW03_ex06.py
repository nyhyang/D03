...
def hypotenuse(leg1, leg2):
    # Using the formula to find out about the length of the hypotenuse
...
def is_between(x, y, z):
    #Comparing the value between three arguments
    if x <= y <= z:
        return True
    else:
        return False
...
def is_palindrome(s):
    #Using len to help identify the type of the variable
    #Why can't I use if type(s) == <type 'str'> to varify?
    if len(s) > 0:
        return True
    else:
        return False
